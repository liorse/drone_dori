...
def Histdata(ans):
    ...
    data['OPC-T'] = Tempcon(combine_bytes(ans[56], ans[57]))
    data['OPC-RH'] = RHcon(combine_bytes(ans[58], ans[59]))
...
def initFile(date):
    ofile = '/home/pi/Documents/OPC' + '_' + str(date).replace('-', '') + ".csv"
    print("Opening Output File:")
    if (not os.path.isfile(ofile)):
        f = open(ofile, 'w+')
        print(
            "time,b0,b1,b2,b3,b4,b5,b6,b7,b8,b9,b10,b11,b12,b13,b14,b15,b16,b17,b18,b19,b20,b21,b22,b23,period,FlowRate,OPC-T,OPC-RH,pm1,pm2.5,pm10,Check", file=f)
    else:
        f = open(ofile, 'a')
    return f
...
if __name__ == "__main__":
    ...
    # time loop
    for n in range(1): #while time.time() % integration != 0:
